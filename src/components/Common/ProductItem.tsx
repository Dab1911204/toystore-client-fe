...
const ProductItem = ({ item }: { item: Product }) => {
  ...
  const hasDiscount = item.promotion &&
    typeof item.promotion === 'object' &&
    'discountedPrice' in item.promotion &&
    item.promotion.discountedPrice != null;

  const discountedPrice = hasDiscount
    ? Number(item.promotion.discountedPrice)
    : Number(item.price);
        ...
        <div className="relative flex h-[280px] items-center justify-center overflow-hidden bg-white">
          ...
          >
            ...
            <Image
              ...
              width={240}
              height={240}
              className={`object-contain transition-transform duration-300 group-hover:scale-[1.15] ${isImageLoading || hasImageError
                ? 'opacity-0'
                : 'opacity-100'
                }`}
          ...
          <div className="mb-3 flex items-center justify-between text-custom-xs text-gray-500">
            ...
            <div className="flex items-center gap-1">
              {item.quantity > 0 ? (
                ...
                <span className="text-red-500 font-medium text-red">Hết hàng</span>
              )}
            </div>
